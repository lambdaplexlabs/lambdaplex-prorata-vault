...
library PRBMathCommon {
    ...
    uint64 public constant MAXINT64 = 2 ** 63 - 1;
    ...
    function mulDivInt64(
        uint256 x,
        uint256 y,
        uint256 denominator
    ) internal pure returns (int64) {
        uint256 tempResult = mulDiv(x, y, denominator);
        if (MAXINT64 >= tempResult) {
            return int64(uint64(tempResult));
        }
        else {
            revert();
        }
